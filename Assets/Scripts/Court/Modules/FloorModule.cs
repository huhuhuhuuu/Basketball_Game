...
{
    /// <summary>
    /// FloorModule – the green asphalt slab the whole court stands on.
    ///
    /// What it builds (everything parented under the "Slab" group):
    ///   • CourtSurface – a Unity Plane scaled to 21 × 37 m with its top at y = 0. This is the
    ///     playing surface and keeps its MeshCollider so balls can bounce on it.
    ///   • SlabBody     – a 21 × 0.12 × 37 m concrete block just under the plane, so the slab edge
    ///     shows ~10 cm above the grass (grass is at y = −0.10) like a real poured slab.
    ///
    /// How the realism is achieved (all procedural, no image files):
    ///   • Asphalt albedo (1024², tiles every 2.5 m): cellular noise makes little stone domes
    ///     (aggregate), Fbm adds grit. Green paint sits on top; on some stone tops the paint has
    ///     worn through to grey stone, and the valleys between stones are darker where dirt collects.
    ///   • Normal map from the SAME height field, so the bumps in the lighting line up with
    ...
    ///     scuff bands along the sidelines, two water-stain blooms with tide marks, hairline cracks
    ///     (cell edges of a large Worley pattern) and lighter chalky patches. Mid-grey (0.5) is
    ///     neutral for a Standard-shader detail map: darker = dirt, lighter = fade.
    ///
    ...
    /// build the mapping from that (see <see cref="PlaneUvFrame"/>).
    /// </summary>
    public static class FloorModule
    {
        ...
        // Stones per texture tile: 2.5 m / 96 ≈ 2.6 cm per stone – outdoor asphalt aggregate.
        const int   PebbleCells = 96;
        // Normal-map strength (spec says 4–6 for a 1024 texture).
        ...
        public static void Build(BuildContext ctx, Transform parent)
        {
            ...
            // The paint colour: court green with a small per-build variation.
            ...
            BuildContext.ApplyRenderer(surface, asphalt, false, true);
        ...
        static GameObject BuildSurfacePlane(BuildContext ctx, Transform slab)
        {
            ...
            // The plane is flat at y = 0 and the slab body right under it already casts the slab's
            // shadow onto the grass, so the plane itself does not need to cast one (saves shadow fill).
            // Material is assigned afterwards (we need the plane's mesh to size the texture tiling).
            return ctx.Primitive(PrimitiveType.Plane, "CourtSurface", slab,
                Vector3.zero, Vector3.zero, scale, null,
                collider: true, castShadows: false, receiveShadows: true);
        }

        /// <summary>Concrete block under the surface: top at y = −0.001, bottom at y = −0.121.</summary>
        static void BuildSlabBody(BuildContext ctx, Transform slab)
        {
            ...
            ctx.Box("SlabBody", slab, center, size, SlabConcreteMaterial(ctx), collider: true);
        }
        ...
        /// <summary>
        /// Concrete for the thin slab edge. The cube's side faces map 0..1 across their whole
        /// length, so we tile 12× along the length and use only a 5 % strip of the texture vertically –
        /// that keeps the texels roughly square on a 12 cm tall × 37 m long face.
        /// </summary>
        static Material SlabConcreteMaterial(BuildContext ctx)
        {
            return ctx.Mats.Get("SlabConcrete", () =>
            {
                ...
                // Slightly darker than fresh concrete: the edge has been rained on for years.
                return MatKit.Make("SlabConcrete", new Color(0.80f, 0.80f, 0.78f), 0.18f, 0f)
                    .WithAlbedo(albedo, 12f, 0.05f);
            });
        }

        /// <summary>Green painted asphalt: tiled albedo + normal, plus the slab-wide detail map.</summary>
        static Material BuildAsphaltMaterial(BuildContext ctx, PlaneUvFrame frame, Color paint, int seed)
        {
            return ctx.Mats.Get("CourtAsphalt", () =>
            {
                ...
                // How many texture repeats fit along each UV axis so one tile is ~2.5 m.
                float alongU = frame.UAlongX ? CourtSpec.SlabHalfX * 2f : CourtSpec.SlabHalfZ * 2f;
                float alongV = frame.UAlongX ? CourtSpec.SlabHalfZ * 2f : CourtSpec.SlabHalfX * 2f;

                // Colour lives in the texture, so the material tint stays white. Dry asphalt is matte.
                return MatKit.Make("CourtAsphalt", Color.white, 0.25f, 0f)
                    .WithAlbedo(albedo, alongU / TileMetres, alongV / TileMetres)
        ...
        /// <summary>Height 0..1: little stone domes (Worley cells) plus fine grit in the binder between them.</summary>
        ...
        static Texture2D DetailMap(PlaneUvFrame frame, int seed)
        {
            ...
            return ProceduralTextures.Create(TexSize, TexSize, (u, v) =>
            {
                ...
                // Dirt is a little warm/brown, not pure grey.
        ...
        {
            // Square noise coordinates (0..1 over 37 m in both axes) so the noise blobs are round.
            ...
            d += (ProceduralTextures.Fbm(su, sv, 64, 2, seed + 8) - 0.5f) * 0.05f;   // fine speckle hides banding
            return Mathf.Clamp(d, 0.2f, 0.8f);
        }
        ...
        static float SunFade(float su, float sv, int seed)
        {
            ...
            return (big - 0.5f) * 0.14f + fade * 0.07f;
        }
        ...
        static float TrafficWear(float x, float z, float su, float sv, int seed)
        {
            ...
            float ftZ      = endInner - CourtSpec.FreeThrowLineDist + CourtSpec.LineWidth * 0.5f; // 8.175
            ...
            for (int end = 0; end < 2; end++)
            {
                ...
                wear += 0.09f * Ellipse(x, z, 0f, basketZ, 2.4f, 1.9f);
                float zMin = Mathf.Min(sign * ftZ, sign * endInner);
                float zMax = Mathf.Max(sign * ftZ, sign * endInner);
                wear += 0.045f * RectMask(x, z, -halfKey, halfKey, zMin, zMax, 0.6f) * wearNoise;
            }
        ...
        static float SidelineScuffs(float x, float su, float sv, int seed)
        {
            ...
            return band * (0.03f + 0.06f * streak);
        }
        ...
        static float Cracks(float x, float z, float su, float sv, int seed)
        {
            ...
            return where * (0.28f * crack - 0.05f * rim);           // dark crack, faint lighter edge
        }
